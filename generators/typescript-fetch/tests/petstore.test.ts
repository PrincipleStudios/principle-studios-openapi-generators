...
const fetchApi = toFetchApi(operations, 'http://localhost', fetch);
...
describe('typescript-rxjs petstore.yaml', () => {
    ...
    it('can wrap two different posts', async () => {
        ...
        expect(response1.response.getResponseHeader('Content-Type')).toBe('application/json');
        expect(response1.data).toEqual(outputPet1);
        expect(response2.statusCode).toBe(200);
        expect(response2.response.getResponseHeader('Content-Type')).toBe('application/json');
    ...
    it('can wrap any post', async () => {
        ...
        const response = await fetchApi.addPet({ body: ({ name: 'Fido', tag: 'dog' }) });
        expect(response.statusCode).toBe(200);
        expect(response.response.getResponseHeader('Content-Type')).toBe('application/json');
