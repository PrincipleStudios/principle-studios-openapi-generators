...
export type AsyncResponseResolver<RequestType = MockedRequest, BodyType extends DefaultBodyType = any> = (req: RequestType, res: ResponseComposition<BodyType>, context: RestContext) => ResponseResolverReturnType<MockedResponse<BodyType>> | Promise<ResponseResolverReturnType<MockedResponse<BodyType>>>;

export type SafeResponse<T extends StandardResponse = StandardResponse> =
    Omit<T, 'response'>
    & { headers?: Record<string, unknown> };

export function toMswResponse(response: SafeResponse, res: ResponseComposition, ctx: RestContext) {
    return res(
...
export class MappedRestHandler<T extends MockedRequest = MockedRequest> extends RestHandler<T> {
    ...
    override run(...params: Parameters<RestHandler<T>['run']>): ReturnType<RestHandler<T>['run']> {
...
>(conversion: RequestConversion<TMethod, TUrlParams, TRequestParams, TRequestBodies, TResponses, TCallType>) {
    ...
    return function (request: MyRequest, response: AsyncResponseResolver | SafeResponse<TResponses>) {
        const standardRequest: AdapterRequestArgs = 'body' in request
            ? conversion.request(request.params, request.body as TRequestBodies[keyof TRequestBodies], request.mimeType as keyof TRequestBodies)
            : {
