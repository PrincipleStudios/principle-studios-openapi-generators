...
/// <see href="https://json-schema.org/draft/2020-12/json-schema-validation#name-maxlength">Draft 2020-12 maxLength keyword</see>
public class MaxLengthKeyword(string keyword, int value) : IJsonSchemaAnnotation
{
	...
	private static DiagnosableResult<IJsonSchemaAnnotation> Parse(string keyword, ResolvableNode nodeInfo, JsonSchemaParserOptions options)
	{
		if (nodeInfo.Node is JsonValue val && val.TryGetValue<decimal>(out var value) && value % 1 == 0 && value >= 0)
			return DiagnosableResult<IJsonSchemaAnnotation>.Pass(new MaxLengthKeyword(keyword, (int)value));
