	...
	public class TypeScriptSchemaTransformerShould
	{
		...
		public static IEnumerable<object[]> InlineAssertionData =>
			new List<(string documentName, Func<OpenApiDocument, OpenApiSchema> findSchema, string expectedInline)>
			{
				...
				("petstore.yaml", (OpenApiDocument doc) => doc.Paths["/pets/{id}"].Operations[OperationType.Get].Parameters.First(p => p.Name == "id").Schema, "number"),
				("petstore.yaml", (OpenApiDocument doc) => doc.Paths["/pets/{id}"].Operations[OperationType.Delete].Parameters.First(p => p.Name == "id").Schema, "number"),
